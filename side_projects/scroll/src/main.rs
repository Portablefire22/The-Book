...
fn main() {
    ...
    // I found out how to create it at runtime
    let mut strings: [[char; 120]; 5] = [[' '; 120]; 5];
    println!("Input your string ({} Char max): ", (strings[0].len()/4));
    
    let mut stri = String::from("") ;
    while stri.len() > (strings[0].len()/4) || stri == ""{
        stri = String::from("");
        io::stdin()
    ...
    loop {
        ...
        for current_row in strings{
            ...
            for _current_char in strings[x]{
                if z != strings[0].len() as i32{ // Stops indexing error, seems to work without any problems
...
fn lookup_char(mut txt: [[char; 120]; 5], input_str: String) -> [[char; 120]; 5]{
